...
class Reports:
    ...
    def high_profit_products_report(self):
        ...
        SELECT 
            m.name,
            AVG(s.total_price / s.quantity) AS avg_selling_price,
            AVG(p.purchase_price) AS avg_purchase_price,
            (AVG(s.total_price / s.quantity) - AVG(p.purchase_price)) AS avg_profit
        FROM sales s
        JOIN medicines m ON s.medicine_id = m.medicine_id
        JOIN purchases p ON p.medicine_id = m.medicine_id
        GROUP BY m.medicine_id
        HAVING avg_profit > 10  -- Example threshold
    ...
    def profit_summary_report(self):
        ...
        SELECT 
            ...
            AVG(s.total_price / s.quantity) AS avg_selling_price,
            AVG(p.purchase_price) AS avg_purchase_price,
            (AVG(s.total_price / s.quantity) - AVG(p.purchase_price)) * SUM(s.quantity) AS total_profit
        FROM sales s
        JOIN medicines m ON s.medicine_id = m.medicine_id
        JOIN purchases p ON p.medicine_id = m.medicine_id
        GROUP BY m.medicine_id
    ...
    def loss_report(self):
        ...
        SELECT 
            ...
            AVG(s.total_price / s.quantity) AS avg_selling_price,
            AVG(p.purchase_price) AS avg_purchase_price,
            (AVG(s.total_price / s.quantity) - AVG(p.purchase_price)) * SUM(s.quantity) AS total_loss
        FROM sales s
        JOIN medicines m ON s.medicine_id = m.medicine_id
        JOIN purchases p ON p.medicine_id = m.medicine_id
        GROUP BY m.medicine_id
